...
class wazuhAPIRequest:
    ...
    def get_agentVulnerability(self,id):
        return requests.get(url=f'https://{self.host}:{self.port}/vulnerability/{id}?limit=100000', headers={"Authorization": f"Bearer {self.token}"},verify=self.use_ssl).json()